...
def detect_header_row_xlsx(xlsx_path: Path) -> int:
    """
...
def load_attempt_names() -> tuple[pd.DataFrame, str]:
...
def load_existing_map() -> tuple[pd.DataFrame, Path]:
    map_path = TRANSI_DIR / "FindName.csv"
    if not map_path.exists():
        empty = pd.DataFrame(columns=["User_Full_Name", "Emp_Name", "Added_On", "User_norm"])
        return empty, map_path
    ...
    if "User_norm" not in m.columns:
        m["User_norm"] = m["User_Full_Name"].astype(str).str.upper().str.strip()
    return m, map_path


# ---------- worklist generation ----------

def emit_worklist(emp_path: Path) -> None:
    ...
    if not existing_map.empty:
        already_mapped = set(existing_map["User_norm"].astype(str))
    ...
    cols = ["User_Full_Name", "Suggested_Emp_Name", "Chosen_Emp_Name", "Flag", "Notes"]
    unmatched[cols].to_csv(out_path, index=False)

    # ALSO create a response copy you will edit (audit-friendly)
    resp_path = TRANSI_DIR / f"FindName_Resp_Worklist_{ts}.csv"
    shutil.copyfile(out_path, resp_path)

    print("[INFO] Attempts source:", att_src)
    print("[INFO] HR source     :", emp_path)
    print("[INFO] Wrote worklist:", out_path)
    print("[INFO] Wrote RESP copy (edit this):", resp_path)
    print("[INFO] Open RESP file, fill Chosen_Emp_Name, set Flag=U, save, then run with --update yes --worklist <RESP path>")


def pick_latest_worklist_path() -> Path:
    # Prefer RESP worklists if present
    resp = sorted(glob.glob(str(TRANSI_DIR / "FindName_Resp_Worklist_*.csv")), key=os.path.getmtime, reverse=True)
    if resp:
        return Path(resp[0])
    wl = sorted(glob.glob(str(TRANSI_DIR / "FindName_Worklist_*.csv")), key=os.path.getmtime, reverse=True)
    if wl:
        return Path(wl[0])
    raise FileNotFoundError("No FindName_Resp_Worklist_*.csv or FindName_Worklist_*.csv found in Transi/.")


# ---------- update mapping ----------

def update_mapping_from_worklist(worklist_path: Optional[Path]) -> None:
    if worklist_path is None:
        wl_path = pick_latest_worklist_path()
    else:
        wl_path = Path(worklist_path)
        if not wl_path.exists():
            raise FileNotFoundError(f"Worklist not found: {wl_path}")
    ...
    to_apply = wl[(wl["Flag"].astype(str).str.upper() == "U") &
                  (wl["Chosen_Emp_Name"].astype(str).str.strip() != "")]
    if to_apply.empty:
        print(f"[INFO] No rows flagged with U and a Chosen_Emp_Name in {wl_path.name}. Nothing to update.")
        return
...
def main():
    ...
    ap.add_argument("--update", choices=["yes", "no"], default="no",
                    help="Apply edited worklist rows (Flag=U) into Transi/FindName.csv")
    ap.add_argument("--worklist", help="Path to a specific worklist CSV (use your RESP file).")
    args = ap.parse_args()

    if args.update == "yes":
        update_mapping_from_worklist(Path(args.worklist) if args.worklist else None)
    else:
