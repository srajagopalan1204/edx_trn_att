...
def _norm_name(x: str) -> str:
    """Robust normalizer for person names: uppercase, strip, collapse spaces, drop periods."""
    if pd.isna(x):
        return ""
    s = str(x).upper().strip()
    s = s.replace(".", " ")
...
def _norm_mod(x: str) -> str:
    """Robust normalizer for module names."""
    if pd.isna(x):
        return ""
    s = str(x).strip()
...
# -------------------------------------------------
# REQUIREMENTS (explicit path or fallback) for Requirement_Matrix sheet
# -------------------------------------------------
...
def build_requirement_matrix_fallback() -> pd.DataFrame:
    ...
    if {"Role_or_Dept", "Module_Name", "ReqFlag"}.issubset(role_map.columns):
        ...
        return pd.DataFrame()  # unknown schema; skip
...
# -------------------------------------------------
# ROLE/AREA/POLICY MAP (legacy fallback) with UNMAPPED reporting
# -------------------------------------------------
def load_role_area_policy_map(
    ...
    """
    Legacy path: load CSV/XLSX linking Module -> Entity / Role_Area / Policy_Normalized.
...
def upsert_dnr_with_terms(dnr_df: pd.DataFrame, terms_path: Path) -> pd.DataFrame:
    ...
    for n in sorted(term_names):
        ...
        if not exists:
            new_row = {
                "User.Full Name": n,  # normalized form is acceptable
...
# ---------- NEW: Module_Risk_By_Policy from Module Catalog ----------
def load_latest_module_catalog(explicit_path: Optional[str] = None) -> tuple[pd.DataFrame, Optional[Path]]:
    """
    Returns (catalog_df, catalog_path) or (empty_df, None).
    Catalog must have columns: Module, Entity, SOP, SubSOP.
    If explicit_path is passed, loads that; otherwise picks the latest module_catalog_*.csv under Outputs/ModMstr.
    """
    out_dir = OUTPUT_DIR / "ModMstr"
    if explicit_path:
        p = Path(explicit_path)
        if not p.exists():
            print(f"[WARN] Module catalog not found at: {p}")
            return pd.DataFrame(), None
        df = _read_csv_or_xlsx(p)
        need = {"Module", "Entity", "SOP", "SubSOP"}
        if not need.issubset(df.columns):
            missing = sorted(need - set(df.columns))
            print(f"[WARN] Module catalog missing columns: {missing}")
            return pd.DataFrame(), None
        df = df.copy()
        df["Module_norm"] = df["Module"].map(_norm_mod)
        for c in ["Entity", "SOP", "SubSOP"]:
            df[c] = df[c].astype(str).str.strip()
        return df, p

    if not out_dir.exists():
        return pd.DataFrame(), None
    candidates = sorted(out_dir.glob("module_catalog_*.csv"))
    if not candidates:
        return pd.DataFrame(), None
    latest = max(candidates, key=lambda p: p.stat().st_mtime)
    df = _read_csv_or_xlsx(latest)
    need = {"Module", "Entity", "SOP", "SubSOP"}
    if not need.issubset(df.columns):
        missing = sorted(need - set(df.columns))
        print(f"[WARN] Module catalog missing columns: {missing}")
...
def build_module_risk_by_policy_from_catalog(enriched_df: pd.DataFrame, catalog_df: pd.DataFrame) -> tuple[pd.DataFrame, pd.DataFrame]:
    """
    Use module catalog to enrich attempts → (Module, Entity, SOP, SubSOP).
    Returns: (risk_df, unmapped_modules_df)
    """
    if catalog_df is None or catalog_df.empty:
        return pd.DataFrame(), pd.DataFrame()

    tmp = enriched_df.rename(columns={"Version Name": "Module"}).copy()
    tmp["Module_norm"] = tmp["Module"].map(_norm_mod)

    cat = catalog_df[["Module_norm", "Module", "Entity", "SOP", "SubSOP"]].drop_duplicates()

    joined = tmp.merge(cat, on="Module_norm", how="left", suffixes=("", "_cat"))

    # Unmapped module names (what attempts used but catalog didn't know)
    unmapped_mask = joined["Entity"].isna()
    unmapped = (
        joined.loc[unmapped_mask, ["Module"]]
        .dropna().drop_duplicates().sort_values("Module").reset_index(drop=True)
    )

    rows = []
    keys = ["Module", "Entity", "SOP", "SubSOP"]
    for _, g in joined.groupby(keys, dropna=False):
        core = _module_risk_core(g)
        rows.append({
            "Module": g["Module"].iloc[0],
            "Entity": g["Entity"].iloc[0],
            "SOP": g["SOP"].iloc[0],
            "SubSOP": g["SubSOP"].iloc[0],
            **core
        })
    risk = pd.DataFrame(rows)
    if not risk.empty:
        risk = risk.sort_values(["Entity", "SOP", "SubSOP", "Module"], na_position="last").reset_index(drop=True)
    return risk, unmapped


# -------------------------------------------------
# LEGACY policy-based (kept for fallback / compatibility)
# -------------------------------------------------
...
def main():
    ap = argparse.ArgumentParser(
        description="Build full training attainment report with HR enrichment, name overrides, policy, requirements, DNR, and module catalog."
    )
    ...
    ap.add_argument("--role-area-policy", help="Path to entity_role_area_policy_normalized_202*.csv/xlsx (legacy fallback).")
    ap.add_argument("--req-matrix", help="Path to role→module requirements (CSV or XLSX) for Requirement_Matrix tab (optional).")
    ...
    # Module catalog (preferred for Module_Risk_By_Policy)
    ap.add_argument("--module-catalog", help="Path to Outputs/ModMstr/module_catalog_*.csv (optional; else latest auto-picked).")
    args = ap.parse_args()
    ...
    emp_cols = [c for c in enriched_df.columns if c.startswith("Emp_")]
    emp_lookup = enriched_df[[user_col] + emp_cols].drop_duplicates(subset=[user_col])
    last_active_view = last_touch.merge(emp_lookup, how="left", on=user_col)
    keep = [
        user_col,
        "Emp_InforID", "Emp_Manager", "Emp_PrimaryFunctionalGroup",
        "Emp_Title", "Emp_RoleName", "Emp_RoleDescription",
    ...
    keep = [c for c in keep if c in last_active_view.columns]
    last_active_view = last_active_view[keep].sort_values(user_col).reset_index(drop=True)

    # 5b) plain Module_Risk
    module_risk_df = build_module_risk(enriched_df)

    # 6) Module_Risk_By_Policy — prefer Module Catalog; fallback to legacy policy
    module_risk_by_policy_df = pd.DataFrame()

    # Prefer explicit CLI path; else try latest under Outputs/ModMstr; else settings override key `module_catalog_override`
    catalog_explicit = args.module_catalog if hasattr(args, "module_catalog") else None
    if not catalog_explicit:
        # allow settings-based override if user wants to lock a catalog path
        catalog_explicit = load_settings().get("module_catalog_override")

    catalog_df, catalog_path = load_latest_module_catalog(explicit_path=catalog_explicit)
    if not catalog_df.empty:
        mrbp, unmapped_mods = build_module_risk_by_policy_from_catalog(enriched_df, catalog_df)
        module_risk_by_policy_df = mrbp
        if not unmapped_mods.empty:
            ts = datetime.now().strftime("%Y%m%d_%H%M")
            out_unmapped = OUTPUT_DIR / f"catalog_unmapped_modules_{ts}.csv"
            unmapped_mods.to_csv(out_unmapped, index=False)
            print("[WARN] Wrote catalog unmapped modules:", out_unmapped)
        if catalog_path:
            print(f"[INFO] Module_Risk_By_Policy built from catalog: {catalog_path}")
    else:
        # Fallback legacy path (role-area-policy + req-matrix)
    ...
    out_xlsx = write_output_excel(
        ...
        emp_path_used=Path(args.emp_mstr) if args.emp_mstr else None,
